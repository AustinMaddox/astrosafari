...
*/	attached_recent_images($forum_id, 5, 'vertical', 32, 200, 201);
	...
	{
		$forum_id_all .= $row['forum_id'] . ',';
	}
	$db->sql_freeresult($result);

	/* RANDOM function
	attached_random_images($forum_ids, $max_limit_arg, $orientation_arg, $num_chars, $max_width, $resize_after)
*/	attached_random_images($forum_id_all, 4, 'vertical', 32, 200, 201);
// ENDED - Attached Images Block mod

// BEGAN - Random Topics by Austin
$sql = array(
	'SELECT'	=> 'f.forum_name, t.topic_title, t.forum_id, t.topic_id, t.topic_type',
	'FROM'		=> array(TOPICS_TABLE => 't', FORUMS_TABLE => 'f'),
	'WHERE'		=> 'f.forum_id = t.forum_id',
	'ORDER_BY'	=> 'RAND()',
);
$result = $db->sql_query_limit($db->sql_build_query('SELECT_DISTINCT', $sql), 10, 0, 30);

while( $row = $db->sql_fetchrow($result) )
{
	$topic_id_rand = $row['topic_id'];
	$forum_id_rand = $row['forum_id'];
	
	$char_limit_rand = 30;

	$view_topic_url_rand = append_sid("{$phpbb_root_path}viewtopic.$phpEx", 'f=' . $forum_id_rand . '&amp;t=' . $topic_id_rand);
	$view_forum_url_rand = append_sid("{$phpbb_root_path}viewforum.$phpEx", 'f=' . $forum_id_rand);
	$topic_title = $row['topic_title'];
	$topic_title_rand = $char_limit_rand > 0 && ( ( $length = utf8_strlen($topic_title) ) > $char_limit_rand ) ? ( utf8_strlen($fragment = utf8_substr($topic_title, 0, $char_limit_rand + 1 - 4)) < $length + 1 ? preg_replace('`\s*\S*$`', '', $fragment) . '...' : $topic_title ) : $topic_title;

	$template->assign_block_vars('random_topics',array(
		'U_TOPIC_RAND' 			=> $view_topic_url_rand,
		'U_FORUM_RAND'			=> $view_forum_url_rand,
		'FORUM_NAME_RAND'		=> $row['forum_name'],
		'TOPIC_TITLE_RAND' 		=> $topic_title_rand,
		'TOPIC_TITLE_RAND_ALT'	=> $topic_title,
	));
}
$db->sql_freeresult($result);
// ENDED - Random Topics by Austin
...
// BEGAN - Share Topic mod
	'U_FACEBOOK_TOPIC'		=> 'http://www.facebook.com/share.php?u=' . generate_board_url() . "/viewtopic.$phpEx?f=$forum_id%26t=$topic_id",
	'U_TWITTER_TOPIC'	   	=> 'http://twitter.com/home?status=' . generate_board_url() . "/viewtopic.$phpEx?f=$forum_id%26t=$topic_id",
	'U_DELICIOUS_TOPIC'	    => 'http://del.icio.us/post?url=' . generate_board_url() . "/viewtopic.$phpEx?f=$forum_id%26t=$topic_id",
	
	'U_DIGG_TOPIC'	    	=> generate_board_url() . "/viewtopic.$phpEx?f=$forum_id%26t=$topic_id",
// ENDED - Share Topic mod
	...
	if (!isset($user_cache[$poster_id]))
	{
		if ($poster_id == ANONYMOUS)
		{
			$user_cache[$poster_id] = array(
				'joined'		=> '',
				'posts'			=> '',
// BEGAN - NV Usertopics mod
				'topics'		=> '',
// ENDED - NV Usertopics mod
				'from'			=> '',
...
// BEGAN - Special and Normal Rank Images mod
				'extra_rank_title'		=> '',
				'extra_rank_image'		=> '',
...
// BEGAN - Annual Stars mod
			$user_cache[$poster_id]['reg_stars'] = '';
// ENDED - Annual Stars mod
...
// BEGAN - Special and Normal Rank Images mod
				...
				'extra_rank_image'		=> '',
				'extra_rank_image_src'	=> '',
// ENDED - Special and Normal Rank Images mod
...
// BEGAN - Annual Stars mod
			$user_cache[$poster_id]['reg_stars'] = '';
			$reg_author_username = $user_cache[$poster_id]['author_username'];
			if ($reg_years = (int) ((time() - (int) $row['user_regdate']) / 31536000))
			{
				$reg_output = sprintf($user->lang['YEAR_OF_MEMBERSHIP'], $reg_author_username, $reg_years);
				if($reg_years > 1)
				{
					$reg_output = sprintf($user->lang['YEARS_OF_MEMBERSHIP'], $reg_author_username, $reg_years);
				}
...
// BEGAN - phpBB-SEO.com Related Topics mod
...
// ENDED - phpBB-SEO.com Related Topics mod
...
// Output the page
// BEGAN - phpBB-SEO Optimal Titles mod
...
// ENDED - phpBB-SEO Optimal Titles mod

// BEGAN - Precise Similar Topics mod
...
// ENDED - Precise Similar Topics mod
