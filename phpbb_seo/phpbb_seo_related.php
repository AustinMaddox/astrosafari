			...
			while($row = $db->sql_fetchrow($result)) {
				...
				if ($auth->acl_get('f_list', $related_forum_id)) {
					...
					$template->assign_block_vars('related', array(
						...
						'U_FORUM' => append_sid("{$phpbb_root_path}viewforum.$phpEx", "f=$related_forum_id"),
						'FORUM' => $row['forum_name'],
						'PAGINATION' => topic_generate_pagination($replies, $view_topic_url),
						'REPLIES' => $replies,
						'TOPIC_REPLIES_ADVERTISEMENT' => rand(5,20),
						'VIEWS' => $row['topic_views'],
						'TOPIC_VIEWS_ADVERTISEMENT' => rand(50,200),
						'FIRST_POST_TIME' => $user->format_date($row['topic_time']),
						'LAST_POST_TIME' => $user->format_date($row['topic_last_post_time']),
						'LAST_POST_TIME_ADVERTISEMENT' => $user->format_date(time(), false, true),
						'TOPIC_AUTHOR_FULL' =>  get_username_string('full', $row['topic_poster'], $row['topic_first_poster_name'], $row['topic_first_poster_colour']),
						'TOPIC_AUTHOR_FULL_ADVERTISEMENT' => get_username_string('full', 1734, 'AstroSafari.com'),
