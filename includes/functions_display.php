...
// BEGAN - NV Advanced Last Topic Titles mod
...
// ENDED - NV Advanced Last Topic Titles mod
...
// BEGAN - Avatar of poster on Index and Viewforum mod
		...
		$forum_last_poster_avatar_alt = sprintf($user->lang['LAST_POSTER_FORUM_AVATAR_ALT'], $row['forum_last_poster_name'], $row['forum_name']);
		
		if ( !empty($row['forum_last_poster_avatar']) )
		{
			$avatar_max_dimensions = 60; // here you can change the max-width you would like to have the avatars displayed
...
// BEGAN - Forum Sponsor mod
		if (isset($row['forum_sponsor_forumlist']) && $row['forum_sponsor_forumlist'])
		{
			if (!$row['forum_sponsor_forumlist_html'])
			{
				$row['forum_sponsor_forumlist'] = generate_text_for_display($row['forum_sponsor_forumlist'], $row['forum_sponsor_forumlist_uid'], $row['forum_sponsor_forumlist_bitfield'], $row['forum_sponsor_forumlist_options']);
			}
		}
...
// BEGAN - Forum Sponsor mod
			'FORUM_SPONSOR'		=> $row['forum_sponsor_forumlist'],
// ENDED - Forum Sponsor mod
...
// BEGAN - Avatar of poster on Index and Viewforum mod
			'LAST_POSTER_AVATAR'		=> ($config['enable_avatar_index']) ? get_user_avatar($row['forum_last_poster_avatar'], $row['forum_last_poster_avatar_type'], $avatar_width, $avatar_height, $forum_last_poster_avatar_alt) : false,
...
function get_user_avatar($avatar, $avatar_type, $avatar_width, $avatar_height, $alt = 'USER_AVATAR', $ignore_config = false)
{
	...
	$avatar_img .= $avatar;
	return '<img src="' . (str_replace(' ', '%20', $avatar_img)) . '" width="' . $avatar_width . '" height="' . $avatar_height . '" alt="' . ((!empty($user->lang[$alt])) ? $user->lang[$alt] : $alt) . '" title="' . ((!empty($user->lang[$alt])) ? $user->lang[$alt] : $alt) . '" />';
}
