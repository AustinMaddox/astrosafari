...
// BEGAN - Forum Sponsor mod
		$user->add_lang('mods/forum_sponsor');

		// Start Forum Sponsor on Viewforum
		if ($action == 'edit')
		{
		...
		// Start Forum Sponsor on Forumlist
		if ($action == 'edit')
		{
			$fs_data_forumlist = $this->get_forum_info($forum_id);

			if (!$fs_data_forumlist['forum_sponsor_forumlist_html'])
			{
				decode_message($fs_data_forumlist['forum_sponsor_forumlist'], $fs_data_forumlist['forum_sponsor_forumlist_uid']);
				$fs_data_forumlist['allow_bbcode_forumlist'] = ($fs_data_forumlist['forum_sponsor_forumlist_options'] & OPTION_FLAG_BBCODE) ? true : false;
				$fs_data_forumlist['allow_smilies_forumlist'] = ($fs_data_forumlist['forum_sponsor_forumlist_options'] & OPTION_FLAG_SMILIES) ? true : false;
				$fs_data_forumlist['allow_urls_forumlist'] = ($fs_data_forumlist['forum_sponsor_forumlist_options'] & OPTION_FLAG_LINKS) ? true : false;
			}
			else
			{
				$fs_data_forumlist['allow_bbcode_forumlist'] = $fs_data_forumlist['allow_smilies_forumlist'] = $fs_data_forumlist['allow_urls_forumlist'] = false;
			}

			$template->assign_vars(array(
				'FORUM_SPONSOR_FORUMLIST'				=> $fs_data_forumlist['forum_sponsor_forumlist'],
				'S_SPONSOR_FORUMLIST_HTML_CHECKED'		=> $fs_data_forumlist['forum_sponsor_forumlist_html'],
				'S_SPONSOR_FORUMLIST_BBCODE_CHECKED'	=> $fs_data_forumlist['allow_bbcode_forumlist'],
				'S_SPONSOR_FORUMLIST_SMILIES_CHECKED'	=> $fs_data_forumlist['allow_smilies_forumlist'],
				'S_SPONSOR_FORUMLIST_URLS_CHECKED'		=> $fs_data_forumlist['allow_urls_forumlist'],
			));

			unset($fs_data_forumlist);
		}
		else if ($action == 'add')
		{
			$template->assign_vars(array(
				'S_SPONSOR_FORUMLIST_BBCODE_CHECKED'	=> true,
				'S_SPONSOR_FORUMLIST_SMILIES_CHECKED'	=> true,
				'S_SPONSOR_FORUMLIST_URLS_CHECKED'		=> true,
			));
		}
// ENDED - Forum Sponsor mod
...
// BEGAN - Forum Sponsor mod
					...
					// Start Forum Sponsor on Forumlist
					$forum_data += array(
						'forum_sponsor_forumlist'			=> utf8_normalize_nfc(request_var('forum_sponsor_forumlist', '', true)),
						'forum_sponsor_forumlist_uid'		=> '',
						'forum_sponsor_forumlist_options'	=> 7,
						'forum_sponsor_forumlist_bitfield'	=> '',
						'forum_sponsor_forumlist_html'		=> request_var('sponsor_parse_forumlist_html', false),
					);

					if ($forum_data['forum_sponsor_forumlist'])
					{
						if ($forum_data['forum_sponsor_forumlist_html'])
						{
							$forum_data['forum_sponsor_forumlist'] = htmlspecialchars_decode($forum_data['forum_sponsor_forumlist']);
							$forum_data['forum_sponsor_forumlist_options'] = 0;
						}
						else
						{
							generate_text_for_storage($forum_data['forum_sponsor_forumlist'], $forum_data['forum_sponsor_forumlist_uid'], $forum_data['forum_sponsor_forumlist_bitfield'], $forum_data['forum_sponsor_forumlist_options'], request_var('sponsor_parse_forumlist_bbcode', false), request_var('sponsor_parse_forumlist_urls', false), request_var('sponsor_parse_forumlist_smilies', false));
						}
					}
// ENDED - Forum Sponsor mod
					...
					$forum_data += array(
						...
						'forum_desc_bitfield'	=> '',
// BEGAN - Forum SEO by Stokerpiller mod
...
// ENDED - Forum SEO by Stokerpiller mod
