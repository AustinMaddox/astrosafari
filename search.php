...
if ($interval && !in_array($search_id, array('unreadposts', 'unanswered', 'active_topics', 'egosearch')) && !$auth->acl_get('u_ignoreflood'))
{
	if ($user->data['user_last_search'] > time() - $interval)
	{
		$template->assign_var('S_NO_SEARCH', true);
//		trigger_error('NO_SEARCH_TIME');
		trigger_error(sprintf($user->lang['NO_SEARCH_TIME_SEC'], ($user->data['user_last_search'] + $interval - time())));
	}
}
...
if ($keywords || $author || $author_id || $search_id || $submit)
{
	...
	$l_search_desc = $l_search_desc_side = $l_search_key = '';
	if ($search_id)
	{
		switch ($search_id)
		{
			...
			case 'active_topics':
				...
				$l_search_desc = $user->lang['ACTIVE_TOPICS_DESC'];
				$l_search_desc_side = $user->lang['ACTIVE_TOPICS_DESC_SIDE'];
				$l_search_key = $user->lang['ACTIVE_TOPICS_KEY'];
				$show_results = 'topics';
				$sort_key = 't';
				$sort_dir = 'd';
			...
			case 'unanswered':
				...
				$l_search_desc = $user->lang['UNANSWERED_DESC'];
				$l_search_desc_side = $user->lang['UNANSWERED_DESC_SIDE'];
				$l_search_key = $user->lang['UNANSWERED_KEY'];
			...
			case 'unreadposts':
				...
				$l_search_desc = $user->lang['UNREAD_DESC'];
				$l_search_desc_side = $user->lang['UNREAD_DESC_SIDE'];
				$l_search_key = $user->lang['UNREAD_KEY'];
				// force sorting
				$show_results = 'topics';
				$sort_key = 't';
				$sort_by_sql['t'] = 't.topic_last_post_time';
			...
			case 'newposts':
				...
				$l_search_desc = $user->lang['NEW_DESC'];
				$l_search_desc_side = $user->lang['NEW_DESC_SIDE'];
				$l_search_key = $user->lang['NEW_KEY'];
				// force sorting
...
// ENDED - phpBB-SEO Optimal Titles mod
	...
	$template->assign_vars(array(
		...
		'SEARCH_DESC'		=> $l_search_desc,

// BEGAN - Additional descriptive content to Topicality search pages
		'FORUM_NAME'		=> $l_search_title,
		'FORUM_DESC'		=> $l_search_desc_side,
		'FORUM_SEO_KEY'		=> $l_search_key,
...
$gallery_block->set_nums(array('rows' => 3, 'columns' => 1, 'comments' => 0, 'contests' => 0));
...
$seo_meta->collect('description', $user->lang['SEARCH_DESC']);
